...
### **01. 対象の Google Cloud プロジェクト、 Region、Zone を設定**
...
次に対象の Qwiklab 画面で指定された Region と Zone を環境変数に設定します。特に指定がなかった場合は us-central1 と us-central1-a を設定してください。**(右辺の [REGION] と [ZONE] を手動で置き換えてコマンドを実行します)**
```bash
